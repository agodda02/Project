...
def write_to_file(pos_list):
    with open("pos_tags.txt", "a") as f:
            for pos in pos_list:
                try:
                    f.writelines(pos[0] + '|' + pos[1] + '\n')
                except:
                    print("Couldn't write " + str(pos))

mydb = db.connect()
mycursor = mydb.cursor()
sql = "select answer from qa_pairs where id > 40654"
...
for rows in mycursor:
    ...
    doc = nlp(lowercase_contribution)
    for token in doc:
        tup = (token.text, token.tag_)
        if tup not in pos_list:
            pos_list.append(tup)
            
    if len(pos_list) >= 2000:
        write_to_file(pos_list)
        pos_list.clear()       
    print(i)
    i+=1
